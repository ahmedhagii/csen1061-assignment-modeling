...
```{r, warning=FALSE, cache=T, comment=NA}
...
```{r, message=FALSE, output=FALSE, cache=T, comment=NA}
...
```{r, message=FALSE, output=FALSE, cache=T, comment=NA}
sonar_data_mapped <- sonar_data
sonar_data_mapped$R <- sonar_data %>% apply(1, function(x) {
	if(x['R'] == 'R') {
		return (1)
	}
	return (0)
})
maxs <- apply(sonar_data_mapped, 2, max) 
mins <- apply(sonar_data_mapped, 2, min)

sonar_data_scaled <- as.data.frame(scale(sonar_data_mapped, center = mins, scale = maxs - mins))
```
...
```{r, message=FALSE, echo=FALSE, cache=T, comment=NA}
...
```{r, echo=FALSE, message=FALSE, cache=T, comment=NA}
...
```{r, message=FALSE, warning=FALSE, output=FALSE, cache=T, comment=NA}
...
### Hepatitis Data
```{r, message=FALSE, warning=FALSE, cache=T, comment=NA}
...
#bagging_res = do_it_10_times(cross_validate, hepatitis_data, ipred::bagging, "Class", 10, p=T, type='class')
#hepatitis_data_matrix <- hepatitis_data_matrix %>% cbind(bagging_res)

hepatitis_data_matrix <- hepatitis_data_matrix[,-1]
colnames(hepatitis_data_matrix) <- c("J48", "randomForest", "svm", "naiveBayes")
```
...
#bagging_res = do_it_10_times(cross_validate, spec_data, ipred::bagging, "Class", 10, p=T, type='class')
#spec_data_matrix <- spec_data_matrix %>% cbind(bagging_res)

spec_data_matrix <- spec_data_matrix[,-1]
colnames(spec_data_matrix) <- c("J48", "randomForest", "svm", "naiveBayes")
```

### Sonar Data

```{r, message=FALSE, warning=FALSE, cache=T, comment=NA}
sonar_data <- read.csv("data/SPECT.test", header=F)
colnames(sonar_data) = c("class", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "F13", "F14", "F15", "F16", "F17", "F18", "F19", "F20", "F21", "F22")
sonar_data2 <- read.csv("data/SPECT.train", header=F)
colnames(sonar_data2) = c("class", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "F13", "F14", "F15", "F16", "F17", "F18", "F19", "F20", "F21", "F22")
sonar_data <- rbind(sonar_data, sonar_data2) 
sonar_data <- sonar_data[sample(nrow(sonar_data)), ]
```
...
J48_res = do_it_10_times(cross_validate, sonar_data, J48, "class", 10, p=F)
sonar_data_matrix <- sonar_data_matrix %>% cbind(J48_res)

randomForest_res = do_it_10_times(cross_validate, sonar_data, randomForest, "class", 10, p=F)
sonar_data_matrix <- sonar_data_matrix %>% cbind(randomForest_res)

svm_res = do_it_10_times(cross_validate, sonar_data, svm, "class", 10, p=F)
sonar_data_matrix <- sonar_data_matrix %>% cbind(svm_res)

naiveBayes_res = do_it_10_times(cross_validate, sonar_data, naiveBayes, "class", 10, p=F)
sonar_data_matrix <- sonar_data_matrix %>% cbind(naiveBayes_res)

#bagging_res = do_it_10_times(cross_validate, sonar_data, ipred::bagging, "Class", 10, p=T, type='class')
#sonar_data_matrix <- sonar_data_matrix %>% cbind(bagging_res)

sonar_data_matrix <- sonar_data_matrix[,-1]
colnames(sonar_data_matrix) <- c("J48", "randomForest", "svm", "naiveBayes")
```

### Final Matrices

```{r, comment=NA, cache=T}
...
algorithm_names <- c("J48", "randomForest", "svm", "naiveBayes")
