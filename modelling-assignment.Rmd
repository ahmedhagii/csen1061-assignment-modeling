...
cross_validate <- function(data, model, class, folds, ntree = NULL, print=FALSE, type=NULL) {
  ...
  for (i in 1:k){
    ...
    if(is.null(ntree)) {
      ...
      print("randomForest")
      fit <- frm %>% formula %>% model(data=data, ntree = ntree, importance=TRUE)
    }
...
```{r}
columns <- paste(sonar_data_scaled %>% dplyr::select(-R) %>% names, collapse = " + ")
res <- cross_validate_NN(data=sonar_data_scaled, class= "R", folds=10,  print=T)
summary(res)[4,]
```
...
```{r, output=FALSE}
# res <- cross_validate(hepatitis_data, J48, "X2", 10)
```
...
```{r, output=FALSE}
res <- cross_validate(spec_data, randomForest, "class", 10, ntree=15)
```
```{r}
summary(res)[4,]
```
